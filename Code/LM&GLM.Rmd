...
mean_score %>% ggplot(aes(x = `covid: (Worldwide)`, y = mean_score)) + 
  geom_point()
```

```{r}
mean_score %>% ggplot(aes(x = date, y = mean_score)) + 
  geom_point()
```

```{r}
data %>% ggplot(aes(x = date, y = `covid: (Worldwide)`)) + 
  geom_point()
```
...
```{r}
library(readr)
daily_deaths_covid_19 <- read_csv("daily-deaths-covid-19.csv", 
    col_types = cols(Date = col_date(format = "%d-%b-%y")))

global_daily_death_rate <- daily_deaths_covid_19 %>%
  group_by(Date) %>%
  summarise(Deaths = sum(Deaths))

global_daily_death_rate %>%
  ggplot(aes(x = Date, y = Deaths)) + 
  geom_point(shape=21, color="#69b3a2", fill="#404080", size=2) +
  geom_line(color="grey") + 
  theme_ipsum() +
  theme(plot.title = element_text(size = 12)) +
  ggtitle("Total Covid-19 Deaths per day") +
  xlab("Date") +
  ylab("Deaths")
```
```{r}
